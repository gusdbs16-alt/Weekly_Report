...
이 프로젝트는 Gemini를 사용하여 기상 레이더, 구름 물리, 최신 AI 트렌드를 분석하고 주간 리포트를 생성하여 이메일로 발송하는 자동화 도구입니다.

## 주요 기능
- Gemini API 기반 최신 논문 및 뉴스 검색 및 요약
- 생성된 모든 URL의 유효성 확인
- HTML 형식의 리포트
- GitHub Actions 연동하여 매주 정해진 시간에 자동 발송(월, 수, 금 09시 30분)

## 설정 방법 (GitHub Secrets)
이 프로젝트를 실행하려면 다음 Secrets을 GitHub Repository 설정에 등록해야 합니다.
